...
func (r *Repository) CreateQuiz(quiz model.Quiz) error {
	if _, err := r.db.Exec(`
		...
		quiz.ID, quiz.Title, quiz.Questions, quiz.Results, quiz.CreatedAt, quiz.TimeToLive, quiz.LinkToQuiz, quiz.UserID); err != nil {
...
func (r *Repository) AddResultToQuiz(quiz model.Quiz) error {
	query := `UPDATE quizzes
		SET title = ?, questions = ?, results = ?, created_at = ?, time_to_live = ?, link_to_quiz = ?, user_id = ?
		WHERE id = ?`
	if _, err := r.db.Exec(query, quiz.Title, quiz.Questions, quiz.Results, quiz.CreatedAt, quiz.TimeToLive, quiz.LinkToQuiz, quiz.UserID, quiz.ID); err != nil {
		return fmt.Errorf("db Exec: %w", err)
	}
